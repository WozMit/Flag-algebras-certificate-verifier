...
class flag:
    def __init__(self, size, edges=[], ftype=[]):
        self.size = size
        self.edges = edges
        self.ftype = ftype
...
def get_multiplication_matrix(flags, G):
    ...
    for ftype in permutations(V, type_size):
        new_flag = flag(G.size, G.edges, list(ftype))
...
for index in range(start - 1, end):
    ...
    if verbose >= 2:
        print("Fixed base graph: size: %d edges: %s ftype: %s" %
              (F.size, str(F.edges), str(F.ftype)))
        print("\nObjective function density:", obj_fun_density)
        print("Inner sum:", inner_sum)

    if maximize:
        print("bound - d(T, F_%d) = Σ<M,X> + slack" % (index + 1))
        print(result, '-', obj_fun_density, '=', inner_sum, '+', slack)
    else:
        print("d(T, F_%d) - bound = Σ<M,X> + slack" % (index + 1))
        print(obj_fun_density, '-', result, '=', inner_sum, '+', slack)
    ...
    if sign*(result - obj_fun_density) == inner_sum + slack:
